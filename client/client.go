...
import (
	"context"
	pb "github.com/DmitriyKolesnikM8O/gRPCJetBrainsGuide/src/go"
...
func createTask(client pb.TaskServiceClient, createTaskRequest *pb.CreateTaskRequest) {
	log.Printf("Creating Task: { description: '%s', user_id: '%s', deadline: '%s' }",
		createTaskRequest.Description, createTaskRequest.UserId, createTaskRequest.Deadline)
	createdTask, err := client.CreateTask(context.Background(), createTaskRequest)
	if err != nil {
		log.Fatalf("Failed to create task: { description: '%s', user_id: '%s', deadline: '%s' }",
			createTaskRequest.Description, createTaskRequest.UserId, createTaskRequest.Deadline)
	} else {
...
func listTasks(client pb.TaskServiceClient, userId string, deadline string) {
	...
	stream, err := client.ListTasks(ctx, &pb.ListTaskRequest{
		UserId:   userId,
